...
export default function Dashboard({ token, user, onLogout }){
  ...
  async function fetchFormats(){
    if (!currentSubject) return;
    try{
      const res = await axios.get(`https://question-1-203z.onrender.com/api/formats/list?subject=${currentSubject}`, { 
        headers: { Authorization: 'Bearer '+token }
      });
  ...
  async function generatePaper(formatId) {
    try{
      const endpoint = generationType === 'individual' 
        ? 'https://question-1-203z.onrender.com/api/individual-paper/generate-with-format'
        : 'https://question-1-203z.onrender.com/api/paper/generate-with-format';
  ...
  async function deleteFormat(formatId) {
    ...
    try {
      await axios.delete(`https://question-1-203z.onrender.com/api/formats/${formatId}`, {
        headers: { Authorization: 'Bearer '+token }
      });
